"""Trains a GANEstimator on Landscape data."""
...
from utils import download_and_convert_landscape
...
def define_flags():
    ...
    tf.flags.DEFINE_string('dataset_dir', './landscape_data/', 'Location of data.')
    tf.flags.DEFINE_string('eval_dir', '/tmp/landscape-estimator/',
                           'Directory where the results images are saved to.')
    tf.flags.DEFINE_string('model_dir', './landscape-model/',
...
def main(_):
    ...
    if not tf.gfile.Exists(FLAGS.dataset_dir):
        ...
        download_and_convert_landscape.run(FLAGS.dataset_dir)

    shape = download_and_convert_landscape.get_shape()
    ...
    gan_estimator = tfgan.estimator.GANEstimator(
        ...
        discriminator_optimizer=tf.train.AdamOptimizer(0.0001, 0.5, use_locking=True),
