...
def parse_fn(record, shape):
    ...
    image = tf.image.resize_images(image, [FLAGS.image_dims]*2, align_corners=True)
...
def provide_data(dataset_dir, shape):
    ...
    """
    dataset = tf.data.TFRecordDataset(os.path.join(dataset_dir, "landscape_train.tfrecord"))
